...
export const voice: Module = {
    ...
    handle: (event, untypedArgs) => {
        ...
        if (args.newState && args.newState.channelId === process.env.MODULE_AUTOMATIC_VOICE_CHANNEL) {
            ...
            args.newState.guild.channels.create({
                name: names[Math.floor(Math.random() * names.length)],
                type: ChannelType.GuildVoice,
                parent: channel?.parent,
            }).then(channel => {
                console.log(`Created channel ${channel.name} (${channel.id}) for ${args.newState.member?.displayName}.`);
                args.newState.setChannel(channel);
            });
        }
